...
This variant describes $states$ consisting of a $stack$ and a $heap$. A $stack$ is a function mapping reference variables to non-addressable values and locations, which are disjoint sets. A $heap$ is a partial function mapping locations to $records$. The $records$ are defined by the user as a set of fields typed as reference or data. The domain of the $heap$ $H$ is denoted by $ldom(H)$. $nil$ is interpreted to a location $S(nil) \notin ldom(H)$. Two $heaps$ are disjoint if their domains do not overlap.
\begin{equation*}
Heaps \buildrel \text{d{}ef}\over= Loc \rightharpoonup (Fields \to Val\cup Loc)
\end{equation*}
\begin{equation*}
Stacks \buildrel \text{d{}ef}\over= Var \to Val \cup Loc
\end{equation*}
...
The logic allows to prove judgments of the form $(S,H)$ $\vDash$ $P$, where $S$ is a stack, $H$ is a heap, and $P$ is an assertion over the given stack and heap. The semantics are given as follows:\\

The benchmarks of SL-COMP use inductive definitions and the general syntax and examples can be referred to the report on SL-COMP.\\
