...
\end{verbatim}
$\mathbb{P}$ is the set of recursive definitions defined by the syntax $P(\mathbf{E}\text{,}\mathbf{D}\text{)}\overset{\Delta}{=}\bigvee_{i}\ \exists X_i,D_i,\Pi_i\wedge \Sigma_i \wedge \Delta_i$ where the spatial formulas $\Sigma_i$ may call $P$ or other predicates from $\mathbb{P}$. In spatial formulas, $emp$ means empty $\mathtt{heap}$, and $junk$ is true for any $\mathtt{heap}$,whether empty or consisting of some allocated nodes. * is the separating conjunction symbol.\\ 　
The logic allows to prove judgments of the form $(S,H)$ $\vDash$ $\varphi$, where $S$ is a $\mathtt{stack}$, $H$ is a $\mathtt{heap}$, and $\varphi$ is an assertion over the given $\mathtt{stack}$ and $\mathtt{heap}$. The semantics are given as follows:
\begin{equation*}
\begin{split}
