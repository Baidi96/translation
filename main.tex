...
This variant describes $\mathtt{states}$ consisting of a $\mathtt{stack}$ and a $\mathtt{heap}$. A $\mathtt{stack}$ is a function mapping reference variables to non-addressable values and locations, which are disjoint sets. A $\mathtt{heap}$ is a partial function mapping locations to $\mathtt{records}$. The $\mathtt{records}$ are defined by the user as a set of fields typed as reference or data. The domain of the $\mathtt{heap\ H}$ is denoted by $\mathtt{ldom(H)}$. Two $\mathtt{heaps}$ are disjoint if their domains do not overlap.$\mathtt{nil}$ is interpreted to a location $\mathtt{S(nil) \notin ldom(H)}$. Their definitions are as follows.
\begin{equation*}
\begin{split}
